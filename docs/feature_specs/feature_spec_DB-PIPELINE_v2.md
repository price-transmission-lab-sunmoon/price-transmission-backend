...
- v2.1 (2026-05-18): §2 Phase 6 입력 컬럼 설명 보완. `borderline_cointegration`·`bai_perron_best_k`(`bp_best_k`)·`bic_scores` 3개 필드가 `db_schema_v5 §breakpoints`에 존재하지 않아 DB 적재 제외됨을 명시 (`phase6.py` docstring과 정합). `app/db/loader/base.py` `upsert_data_freshness` UPDATE→INSERT 패턴 수정 및 `runner.py` `append_phase_to_run` 누적 호출 추가.  
**스프린트**: S3 (05.05 ~ 05.12)  
**상태**: 초안 / PM 승인 대기
