...
CREATE TABLE stat_timeseries (
    ...
    spread_n2               BOOLEAN,                 -- 스프레드 N=2 판정 (B 구간만)
    spread_n3               BOOLEAN,                 -- 스프레드 N=3 판정 (B 구간만)
    spread_n6               BOOLEAN,                 -- 스프레드 N=6 판정 (B 구간만)
...
CREATE TABLE anomaly_results (
    ...
    ml_consensus_count      SMALLINT     NOT NULL DEFAULT 0,  -- 0~3 (탐지 모델 수)
    if_anomaly              BOOLEAN,
    lof_anomaly             BOOLEAN,
    svm_anomaly             BOOLEAN,
...
CREATE TABLE asymmetry_results (
    ...
    -- 보조 지표 (pipeline_output_spec_vN §pattern2_asymmetry)
    mean_tr_up              NUMERIC(12,6),   -- 상승기 평균 전이율
    mean_tr_down            NUMERIC(12,6),   -- 하락기 평균 전이율
    n_up                    INTEGER,         -- 상승기 관측 수
    n_down                  INTEGER,         -- 하락기 관측 수
...
CREATE TABLE ml_scores (
    ...
    -- 앙상블
    ml_consensus_count  SMALLINT,        -- 0~3 (탐지 모델 수)
...
*v5 — 당시 pipeline_output_spec v6 반영. v5 → v6 (pipeline_output_spec v9 반영): `ml_scores.ml_vote` → `ml_consensus_count` 컬럼명 정합, `anomaly_results.ml_vote` → `ml_consensus_count` 컬럼명 정합, `asymmetry_results`에 `mean_tr_up / mean_tr_down / n_up / n_down` 추가, `stat_timeseries.spread_n2/n3/n6` 타입 `NUMERIC(12,6)` → `BOOLEAN` 수정. (외부 참조 표기 `abcd_vN.md` 규칙, `docs/docs_manifest.md` 버전 해석기 연동)*

---

## 부록 — Phase 7-ML 적재 정책 (backend_reply_phase7ml_v2, 2026-05-24)

### `ml_scores` percentile 산출 규칙

`ml_scores.if_percentile / lof_percentile / svm_percentile`은 파이프라인 산출이 아닌 **백엔드 적재 시점에 산출**한다 (Q1 옵션 A 채택).

**산출 단위**: `(commodity_id, segment_id)` 그룹.

**산출식** (3종 score 모두 동일 방향 — IF/LOF/SVM 전부 `lower score = more anomalous`):

```python
df["if_percentile"]  = df.groupby([cid, seg])["if_score"].rank(pct=True, ascending=False) * 100
df["lof_percentile"] = df.groupby([cid, seg])["lof_score"].rank(pct=True, ascending=False) * 100
df["svm_percentile"] = df.groupby([cid, seg])["svm_score"].rank(pct=True, ascending=False) * 100
# Equivalent: (1 - rank(pct=True, ascending=True)) * 100
```

**의미**: 산출 후 모든 percentile은 **"높을수록 이상"** 으로 통일된다. 예시:
- anomaly 행: 보통 90~99 구간 분포.
- 정상 행: 0~50 구간 분포.

**Tie 처리**: pandas `rank` 기본값(`method='average'`) — 동점 시 평균 순위.

**NULL 처리**: 점수 자체가 NULL이면 percentile도 NULL. segment 내 관측치 1건만 있는 경우 percentile=0.0.

### `ml_scores` boolean 컬럼 정책

`if_anomaly`, `lof_anomaly`, `svm_anomaly`는 DB 컬럼 정의상 nullable이나, **적재 시 항상 NOT NULL 값**으로 강제한다 (Q2 옵션 A 채택). NaN/None → False 변환.

이는 `anomaly_results.if_anomaly / lof_anomaly / svm_anomaly`에도 동일 적용된다.

### `ml_projections` (OI-15) 해소 — 확정 / ③ PR 적용

- `projection_method='pca'` 채택. `x_label="PC1"`, `y_label="PC2"` 고정.
- 산출: features CSV 6피처(`transmission_rate`, `upstream_pct`, `downstream_pct`, `ect_or_spread`, `exchange_rate_pct`, `intl_price_usd_pct`) → `StandardScaler` → `sklearn.decomposition.PCA(n_components=2, random_state=42)`.
- 적재 단위: `(cid, seg, period)` × 3 `model_name` (`isolation_forest`/`lof`/`ocsvm`) = **3행**. 좌표 동일, `anomaly_score`/`is_anomaly`는 모델별 상이 (`if_score/lof_score/svm_score`, `if_anomaly/lof_anomaly/svm_anomaly`).
- segment 내 유효 관측치 2건 미만이면 해당 segment skip.
- `is_anomaly`는 항상 boolean (`*_anomaly` NaN → False 강제 — backend_reply_phase7ml_v2 §2.2 일관).
- `feature_direct` 옵션은 본 PR 미구현 (literal 타입에만 정의).
