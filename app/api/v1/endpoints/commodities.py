...
from app.cache.redis import cached_or_compute
...
async def get_stream(
    ...
    async def _compute() -> StreamResponse:
    ...
    return await cached_or_compute(redis, cache_key, StreamResponse, _compute)
...
async def get_raw_prices(
    ...
    async def _compute() -> RawPricesResponse:
        commodity = await ref_svc.get_commodity_detail(db, commodity_id)
        analysis_start, analysis_end = _analysis_dates(commodity, commodity_id)
    ...
    return await cached_or_compute(redis, cache_key, RawPricesResponse, _compute)
