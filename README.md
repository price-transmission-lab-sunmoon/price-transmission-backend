...
```bash
curl http://localhost:39745/api/v1/meta/config      # → db_status:"ok", redis_status:"ok"
curl http://localhost:39745/api/v1/commodities       # → 품목 10개 (데이터 복원 확인)
```

또는 브라우저에서 Swagger UI: `http://localhost:39745/docs`

### 주의

- **호스트 포트는 `39745`** — compose가 `39745→8001`로 매핑(발표 시연용 선점). 컨테이너 내부 포트는 `8001`이다. 호스트(브라우저·curl)에서는 항상 `39745`로 접근한다.
...
| 메서드 | 경로 | 설명 |
|---|---|---|
| GET | `/anomalies/summary` | 이달의 이상 요약 배너 (스텁 — phase7-stat 실연결 전, 현재 0건 반환) |
| GET | `/anomalies/{id}/detail` | 패널 상세 (stat_metrics + ml_summary + judgment_path) |
