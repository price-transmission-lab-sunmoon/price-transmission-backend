"""Phase 7 이상 패턴 탐지 공통 기반 모듈. 데이터 로딩, 전이율 산출, 하위 기간 판별 기능을 제공한다."""
...
# 분모 폭발 방지: 상류 변화율 절대값이 이 값 미만이면 전이율 NaN 처리
TRANSMISSION_RATE_MIN_UPSTREAM = 0.5

# 동시점 미세 변동은 노이즈이므로 방향 역전 판정에서 제외
DIRECTION_REVERSAL_MIN_MAGNITUDE = 1.0

PATTERN1_SEGMENTS = ["A", "B", "C", "D", "D_prime"]
PATTERN2_SEGMENTS = ["A", "B"]
PATTERN3_SEGMENTS = ["B"]
ASYMMETRY_SEGMENTS = ["A", "B"]
...
def load_changes(paths, cid):
    """Phase 1 변화율 CSV 로드. 반환: DatetimeIndex(MS) DataFrame."""
...
def load_seasonal_adjusted(paths, cid):
    """Phase 1 계절 조정 수준 데이터 CSV 로드. 반환: DatetimeIndex(MS) DataFrame."""
...
def load_baseline(paths, cid, seg):
    """Phase 4 기준선 JSON 로드. normal_transmission_lag, transmission_elasticity 포함."""
...
def load_ect(paths, cid, seg):
    """Phase 4 ECT/로그 스프레드 CSV 로드. ect, ect_type 컬럼 포함."""
...
def load_irf(paths, cid, seg):
    """Phase 4 IRF 시계열 CSV 로드. horizon, irf_downstream 컬럼 포함."""
    filepath = paths.irf_dir / f"{cid}_{seg}_irf.csv"
    return pd.read_csv(filepath, encoding="utf-8-sig")


def load_breakpoints(paths, cid, seg):
    """Phase 6 구조 변화 JSON 로드. bai_perron_breakpoints, subperiods 포함."""
...
def load_subperiod_model(paths, cid, seg, subperiod_id):
    """Phase 6 하위 기간 재추정 모형 JSON 로드. 파일 미존재 시 None 반환."""
    filepath = (
...
class SubperiodResolver:
    """
    특정 날짜가 속한 하위 기간(subperiod)을 판별하고 해당 기준선을 반환한다.
    구조 변화가 없으면 전체 기간 baseline을 그대로 사용한다.
    """
    ...
    def get_normal_lag(self, date):
        """해당 날짜의 정상 전달 시차 반환. subperiod_model 없으면 전체 기간 기준선 폴백."""
    ...
    def get_subperiod_id(self, date):
        """해당 날짜의 subperiod ID 반환. 구조 변화 없으면 None."""
        if not self.has_structural_break:
            return None
...
def compute_transmission_rate(upstream_pct, downstream_pct,
    ...
    """
    월별 전이율 산출 (downstream_pct / upstream_pct).
    상류 변화율 절대값이 min_upstream 미만이면 NaN 처리.
    """
...
def get_pct_columns(config, cid, seg):
    """
    product_config에서 해당 구간의 상류/하류 원본 컬럼명과 _pct 컬럼명을 반환한다.
...
def get_warmup_end(baseline):
    """warmup_end 이전은 롤링 윈도우 분포 축적 기간이므로 패턴 2 탐지를 수행하지 않는다."""
